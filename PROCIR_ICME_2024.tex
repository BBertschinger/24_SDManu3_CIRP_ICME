...
acquisition costs. An alternative to internal sensor system are external sensors, such laser trackers and tracking stations \cite{Moeller17, Yang17}. The costs of these systems make them unsuitable for wide-spread production applications, but are rather used as means for calibration. Instead a holistic approach is needed, as detailed below.
%
...
%\vspace*{8pt}
%\setlist{nolistsep}
%   Softare defined motion planing based on metrological error
% 	Compromise between accuracy & speed -> mp: path planing (kinematic Strategies) & trjacetory generation (dynamci Strategiees)
%   # Hardware Implementation
%    	Trifocal Sensor /  CAN-Bus  / sync. meas of camera, joint angles and joint torque / 
%   # Kinematic Strategies
% 		geometric errors -> offline calibration techniques-> error measure in task space  // measured endeffector pose -> kinematic model % 		able to measure static defomrmation 
%   # Path Planing
%   	Optimal repeatability depends worpiece placement / Two robot approach (one holding - one moving) /  4 Step process / 
%   # Dynamic Strategies
% 		forces 6 torques -> traking errors prdominant in joint space -> compensation Gaussian Process dynamic model  to predict dynamic
% 		erors  / tradeoff accuracy and speed
%   # Trajectory Generation
% 		 and accuracy / Combination optimal control problem (optimize travel time) ->  extended via data-driven error model (based on %
%		 covariance) also optimizes accuracy
%   # Camera Placement
%        minimize obstructions and low measurement accuracy -> show the benefit of a unified error model -> no trade off between fiel of
%		% view and smallest singulat value
%
% # Process Perception
%	New Sensor Multipoint / Increase in 2D Resolution / Find Solution f. Correlation Problem w. Regards to Triangulation Error for multicamera system
% 	# Metrological Error
%		Multicamera System -> Composed of Nsub sStereoscopic cameras -> Combination of error elipsoids
%   # Optimized Perception
%   	Covariance exists -> Simulation Procedure for Point estimate accuracy, Combination of cameras under consideration of
%		measurement error leads to increas in accuracy -> number of obstruction needs to be minimized during camera placemenz
% # Process Strategies
%
%        
% # Conclusion
...
%\rem{The results with experimental data from the real robot are depicted in Fig.~\ref{fig:GP} and show that the dynamic error model is able to predict the errors at other velocity profiles, indicating that the GP has learned systematic dynamic errors despite the complex error sources.
...
Therefore a metrological error model for a photogrammetric sensor systems was presented and kinematic and dynamic strategies
for motion planing were derived. By the means of robot-robot collaboration and path segmentation the kinematic error was minimized. This Path was then translated into a trajectory by minimizing the travel time. On the basis of the GP dynamic model and the covariance of triangulation the system accuracy can be further extended. Finally a placement strategy based on said trajectory was introduced, which minimizes obscurance and avoids areas of low accuracy. By this means the authors are confident to achieve similar levels of accuracy compared to today's laser tracker and tracking stations.
%
\section{Outlook}
