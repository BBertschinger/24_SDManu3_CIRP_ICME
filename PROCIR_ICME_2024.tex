...
Subsequently the relation between for triangulation with multiple cameras can be defined accordingly:
...
In case the extrinsic and intrinsic camera paramters are fully known, the system can be solved by the least-square techniques \textit{add soures}.\\

The solution for the correspondace between object position an image points has been found in Eqn. \ref{eqn:IOMatrix}.  It can be also written in implicit form:
\begin{align}
	f(\hat{\mathbf{Q}},\hat{\mathbf{M}}) \\
	\mathbf{Q} = [\mathbf{P},\mathbf{C}]^{\top}
\end{align}
Whereby the vector of the input Quantities $\hat{\mathbf{Q}} = \left[q_1,\dots, q_{n}\right]^{\top}$ is composed of the vector of image points $\hat{\mathbf{P}} = [x'_1,y'_1, \dots ,x'_n,y'_n]^{\top}$ and the vector of camera parameters, $\hat{\mathbf{C}} = \left[ \mathbf{C}_1 , \dots , \mathbf{C}_n \right]^{\top}$, which in turn define the object point coordinates $\hat{\mathbf{M}} =  [x,y,z]^{\top}$ in cartesian form. \\
...
Whereby $\mathbf{J}$ is also known as Design-Matrix or Jacobian Matrix of the partial derivatives of $f$ with respect to the input quantities $\hat{\mathbf{Q}}$ and $\hat{\mathbf{\Lambda}}$ is the covariance Matrix of said  input values. Both Jacobian $\mathbf{J}$ and Covariance Matrix The Jacobian Matrix $\mathbf{J}$ and the Covariance Matrix $\mathbf{\Lambda}$ can also be written in Tensor notation, whereby the rank of the tensor corresponds to the number of cameras in the system. \\
Both Matrices can be established by a calibration process whereby a series of known markers is placed in the object space \cite{Luhmann2003}. The minimum number of marker needed corresponds to:
...
The increasing requirements for the means of calibration per camera and the corresponding increase of the rank of Jacobian- and Covariance Matrix make it evident that a less elaborant approach is needed.
...
\begin{equation}
	\mathbf{J_{M}^{\Delta}} = \left( \mathbf{J_{M}^{\top}} \mathbf{J_{M}^{}} \right)^{-1}\mathbf{J_{M}^{\top}}
\end{equation}

\begin{equation}
	\mathbf{E} = \mathbf{\Lambda} =
	\begin{bmatrix}
...
\begin{equation}
	\label{eqn:CovarianceMatrix}
	n_{sys} = \mathlarger{\sum}_{n=1}^{N_{cam}}a_{n}-1
\end{equation}
