...
\end{equation}
$\mathbf{J_{Q}}$ is known as design or Jacobi matrix of the partial derivatives of $g(\mathbf{Q},\mathbf{M}$ with respect to the input quantities $\mathbf{Q}$, and $\mathbf{\Lambda_Q}$ is the covariance matrix of said input values. The uncertainty of the input parameters is directly tied to the uncertainty of the output parameters \\
\begin{equation}
...
Whereby $u_n$,$u_p$,$u_p$ is the minimum number of unknown parameters to be resolved and $n_{pictures}$,$n_{points}$,$n_cameras$, is the 
number of pictures, points per picture and cameras within the system.\\
...
\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.9\columnwidth]{graphics/OpticalSimulation.eps}
...
Therefore, a strategy to place the cameras is presented \prop{in the later part of the next section.}
\rem{as a part of the process strategies in the next section.}
%
\section{Process Strategies}
Besides the aforementioned placement of the cameras, incorporating \rem{their measurements} \prop{the metrological error estimate} also opens up new possibilities for a software-defined motion planning of the robot.
...
\rem{These steps are outlined in this section.} \note{remarks on section content not needed, only if anticipation highlights cross-relations.}
...
The optical measurement system is set up with the trifocal sensor according to section~\ref{error_estimate}.
Methods to calibrate the camera system, which exceed the scope of this paper, are described in~\cite{Hartley2018}.
...
The disadvantage, however, is that the drive dynamics are slower than of electric drives. There occur strong friction effects, which means that they have a similar effect on precision as gears in electric drives, so that for both robot types there result dynamic errors in the joint angles. 
...
To this end, the endeffector pose is measured with an external measurement statically at certain points in the workspace a certain number of times, and the geometric error parameters are determined in an optimization.
This procedure does not generalize well in the entire workspace and is not applicable during the industrial robot application itself, but provides a first enhanced kinematic model of the robot.
...
Such calibration also captures the static deformation of the robot structure, which is however load dependent, so that an a priori calibration measurement may not generalize well, which is another motivation for adding an external optical measurement system during robot operation.


=======
Once the configuration of the robot is known from the motion planning steps, also the placement of the cameras for the process perception can be optimized.\rem{as also described in this section.} \note{see note beforehand}
%as also described in this section. 
%
%
\subsection{Hardware implementation}
%
...
\rem{This offline calibration is usually performed at certain points, because of which it does not generalize in the entire workspace and is not applicable in the industrial robot application itself, but provides a first enhanced kinematic model of the robot. From a kinematic point of view, if the manufacturing tolerances of the robot are known, incorporating them in the forward kinematics yields an error measure in task space. Such errors are usually much larger than the measurement accuracy of the presented measurement system.}
\prop{This offline calibration is usually performed at certain point and does not generalize the entire workspace.
	The resulting model is not suited for industrial robot application, but provides a first enhanced kinematic model.
	Incorporating the apriori manufacturing tolerances into the forward kinematic model yields an error measure in the task space.
	It is reasonable to assume these errors are usually much larger than the accuracy of the presented measurement system.}
%
\prop{ It also captures the static deformation of the robot structure, which cannot be separated under different loads}
%
>>>>>>> main
...
This means that we can use \prop{a second workpiece holding robot} \rem{the workpiece holding robot} to reposition the workpiece in such a way that the optimal repeatability is achieved.
It might be tempting to try to find a continuous trajectory of this second robot to minimize the error.
However \prop{this machine} \rem{the second robot} also suffers under finite joint precision, while moving these will introduce additional errors.
It is therefore better if the workpiece holding robot moves to a fixed position before the \prop{primary} \rem{second} robot starts moving.
...
Resulting trajectory tracking errors can be captured in the optical measurement of the TCP, and for example be compensated by a classical task space controller~\cite{Siciliano09}.
...
Since hysteresis occurs in the drives and dynamic effects are to be capture, not only the current position, velocity and acceleration state of the drive but also the motion history several samples back is included. 
...
The results with experimental data from the real robot are depicted in Fig.~\ref{fig:GP} and show that the dynamic error model is able to predict the errors at other velocity profiles, indicating that the GP has learned systematic dynamic errors despite the complex error sources.
A correlation analysis shows that the error depends mostly on the history of the velocity and acceleration, indirectly capturing the dynamic effects in the drive.  
%
From the collected measurements, it becomes clear that at low velocities the stick-slip effect in friction affects the accuracy, while at high velocities the limited bandwidth of the controller implies that tracking errors cannot be compensated fast enough, which also affects accuracy.
Therefore, a medium velocity is to be preferred.
...
After the path has been determined in section~\ref{subsec:PathPlanning} under the premise of optimal repeatability, in this section a trajectory is generated from it under the premise of maximizing the robot’s travel speed and thus the process speed.
To this end, the already determined path of all joint angles is parameterized by a path parameter, describing the paths of all individual robot joints by one single parameter, whereby the individual paths are implicitly synchronized.
Since the robot applied in this work has the mathematical property of differential flatness~\cite{Hoffmann23}, the state of the robot can be represented with the help of the trajectory of the individual joint angles and the path parameter and its time derivatives.
...
A more detailed description of the optimization-based trajectory generation framework is given in~\cite{Kanagalingam24}.
 
This formulation allows for extending the optimization problem from purely travel time and/or energy to also incorporating accuracy.
=======
...
%
\subsection{Trajectory Generation}
\rem{After the path has been determined in section~\ref{subsec:PathPlanning} under the premise of optimal repeatability, in this section a trajectory is generated based on the given path under the premise of maximizing the robot’s travel speed and thus the process speed.
To generate the trajectory the already determined path of all joint angles is parameterized by a path parameter, so that the paths of the individual joints of the robot can be described by this parameter, whereby the individual paths are implicitly synchronized.} \note{Sentence structure is too complex}\\
\prop{In section~\ref{subsec:PathPlanning} the path has been determined under the premise of optimal repeatability. Hereon a trajectory is generated to of maximize the robot’s travel speed and thus the process time.
...
Since the robot applied in this work has the mathematical property of differential flatness~\cite{Hoffmann23}, the state of the robot can be represented with the help of the given trajectory of the individual joint angles \prop{,} \rem{and} the path parameter and its time derivatives.\\
In order to generate the trajectory, it is now sufficient to determine the time derivatives of the path parameters by means of an optimal control problem\prop{. Hence} \rem{in such a way that} the travel time \rem{is} \prop{can be} minimized, taking into account the dynamic, state-dependent constraints of the robot. For a more detailed description of trajectory generation, please refer to~\cite{Kanagalingam24}.
%
\rem{This existing formulation allows for extending the optimization problem from purely travel time and/or energy to also incorporating accuracy.\\}
\prop{This existing formulation can not only optimize travel time and/or energy efficiency, but also the accuracy.\\}
>>>>>>> main
To this end, the data-driven error model from section~\ref{subsec:dynError} can be added to the optimal control problem.
To quantify the errors, the mean value function of the Gaussian process is added while the covariance serves to \rem{quantify} \prop{assess} the certainty of the model \rem{, which can additionally be included in the cost functional.} \prop{Additonaly they can be included in the cost funtion to improve the trajectory generation even further.} Therefore, future work will include optimization-based trajectory generation with this data-driven component added to the optimal control problem.
%
\subsection{Camera Placement}
With the robot configuration being known from the previous steps, the perception of the process can be optimized\prop{:\\} \rem{as described in the following.}
...
\end{figure}
%
\rem{It also has practical advantages.} \note{What advantages and are they relevant in this context?}
